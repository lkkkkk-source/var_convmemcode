...
def build_vae_var(
    ...
    drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,  # dropout rates
    ...
    heads = depth
    width = depth * 64
    dpr = drop_path_rate if drop_path_rate > 0 else 0.1 * depth/24
    ...
    var_wo_ddp = VAR(
        vae_local=vae_local,
        num_classes=num_classes, depth=depth, embed_dim=width, num_heads=heads, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, drop_path_rate=dpr,
