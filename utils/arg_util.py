...
def _get_git_info():
    """Get git info with fallback to [unknown] if not in git repo"""
    try:
        branch = subprocess.check_output(f'git symbolic-ref --short HEAD 2>/dev/null || git rev-parse HEAD', shell=True).decode('utf-8').strip() or '[unknown]'
    except (subprocess.CalledProcessError, FileNotFoundError):
        branch = '[unknown]'

    try:
        commit_id = subprocess.check_output(f'git rev-parse HEAD', shell=True).decode('utf-8').strip() or '[unknown]'
    except (subprocess.CalledProcessError, FileNotFoundError):
        commit_id = '[unknown]'

    try:
        commit_msg = (subprocess.check_output(f'git log -1', shell=True).decode('utf-8').strip().splitlines() or ['[unknown]'])[-1].strip()
    except (subprocess.CalledProcessError, FileNotFoundError):
        commit_msg = '[unknown]'

    return branch, commit_id, commit_msg
...
class Args(Tap):
    ...
    # get git info (defined at module level to avoid class initialization issues)
    _git_branch, _git_commit_id, _git_commit_msg = _get_git_info()
    branch: str = _git_branch  # [automatically set; don't specify this]
    commit_id: str = _git_commit_id  # [automatically set; don't specify this]
    commit_msg: str = _git_commit_msg  # [automatically set; don't specify this]
    ...
    # environment
    out_dir: str = ''  # custom output directory name; if empty, uses exp_name
...
def init_dist_and_get_args():
    ...
    _dir_name = args.out_dir if args.out_dir else (args.exp_name if args.exp_name else 'local_output')
    args.local_out_dir_path = os.path.join(_project_root, 'local_output', _dir_name)
